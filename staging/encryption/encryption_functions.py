...
def encrypt_data(path_to_data, passphrase, move=True):
    if os.path.isdir(path_to_data):
        ...
        for subdir, dirs, files in os.walk(temp_dataset):
            for dir_coll in dirs:
                full_path = os.path.join(subdir, dir_coll)
                abs_path = full_path.replace(temp_dataset, '')
                target_dir = final_output + abs_path
                os.mkdir(target_dir)
            for filename in files:
                enc_file = os.path.join(subdir, filename)
                abs_file = enc_file.replace(temp_dataset, '')
                obj = final_output + abs_file
                command = openssl_path + "openssl enc -e -" + enc_mod + " -a -salt -p -md " + \
                          hash_md + " " + openssl_key_deriv + " -in " + enc_file + " -out " + obj + " -pass stdin"
                ...
                while True:
                    ...
                    if popen.poll() is not None:
                        sys.stdout.write("Finished encrypting:" + enc_file)
                        break
                popen.wait()
                if popen.returncode != 0:
                    sys.stdout.write("Problem creating zip files: " + enc_file)
    else:
